...
py::class_<Topology , Topology_Overloads , std::shared_ptr<Topology >  , TopologicalQuery  >(m, "Topology")
        ...
        /*.def(
            "Type",
            (int(Topology::*)() const)& Topology::Type,
            " ")*/
        .def(
            "SubTopologies", 
        ...
        .def(
            "Navigate",
            [](const Topology& obj, ::TopologicCore::Topology::Ptr const& kpHostTopology, py::list& rMembers) {
                std::list<Topology::Ptr> local;
                obj.Navigate(kpHostTopology, local);
                for (auto& x : local)
                    rMembers.append(x);
            },
            " ", py::arg("kpHostTopology"), py::arg("rMembers"))
        /*.def(
            "UpwardNavigation",
            [](const Topology& obj, ::TopoDS_Shape const & rkOcctHostTopology, int const kTopologyType, py::list& rAncestors) {
                std::list<Topology::Ptr> local;
                obj.UpwardNavigation(rkOcctHostTopology, kTopologyType, local);
                for (auto& x : local)
                    rAncestors.append(x);
            },
            " ", py::arg("rkOcctHostTopology"), py::arg("kTopologyType"), py::arg("rAncestors"))
